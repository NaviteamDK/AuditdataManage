codeunit 50101 "ADM API Client"
{
    ...
    begin
        EnsureSetup();
        HttpRequestMessage.GetHeaders().Add('ApiKey', IntegrationSetup."API Key");
        if IntegrationSetup."EDI Scheme" <> '' then
            HttpRequestMessage.GetHeaders().Add('EdiScheme', IntegrationSetup."EDI Scheme");
        HttpRequestMessage.GetHeaders().Add('Accept', 'application/json');
    ...
    begin
        ...
        repeat
            ...
            if RelativePath.Contains('?') then
                RelativePath += StrSubstNo('&Page=%1&PerPage=%2', Page, PageSize)
            else
                RelativePath += StrSubstNo('?Page=%1&PerPage=%2', Page, PageSize);
