codeunit 50107 "ADM Sale Sync"
{
    ...
    begin
        ...
        if not TrySyncSales(IntegrationSetup, Processed, Failed) then begin
    ...
    local procedure TrySyncSales(IntegrationSetup: Record "ADM Integration Setup"; var Processed: Integer; var Failed: Integer): Boolean
    var
    ...
    begin
        ...
        if not ADMAPIClient.TryGet(
            StrSubstNo('api/v2/invoicing/sales/%1', SaleIDText),
    ...
    begin
        if not ADMAPIClient.TryGet(
            StrSubstNo('api/v2/invoicing/sales/%1/products', SaleIDText),
            ResponseText, ErrorText)
        then
            exit;
